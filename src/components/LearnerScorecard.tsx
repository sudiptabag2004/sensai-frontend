...
const LearnerScorecard: React.FC<LearnerScorecardProps> = ({
    ...
    return (
        <div className={`pt-6 ${className}`}>
            {/* Summary card */}
            <div className="bg-zinc-900 rounded-xl p-5 shadow-sm mb-6">
                <h2 className="text-lg font-light mb-4 text-white">Performance Summary</h2>

                {/* Overall score */}
                <div className="flex items-center justify-between mb-5">
                    <div>
                        <div className="text-sm text-zinc-400">Overall Score</div>
                        <div className="text-2xl font-light mt-1 text-white">{totalScore}/{totalMaxScore}</div>
                    </div>
                    <div className="w-16 h-16 rounded-full flex items-center justify-center relative">
                        <svg className="w-16 h-16 -rotate-90" viewBox="0 0 36 36">
                            <circle cx="18" cy="18" r="16" fill="none" className="stroke-zinc-800" strokeWidth="2"></circle>
                            <circle
                                cx="18"
                                cy="18"
                                r="16"
                        ...
                        <div className="absolute text-sm font-medium text-white">{overallPercentage}%</div>
                    </div>
                </div>
                        ...
                        return (
                            <div key={`summary-${index}`} className="flex items-center justify-between">
                                ...
                                <div className="flex items-center space-x-3 flex-shrink-0">
                                    ...
                                    <span className="text-xs w-10 text-right text-white">{item.score}/{item.max_score}</span>
                                </div>
                            </div>
                        );
                    })}
            ...
            <h3 className="text-base font-light mb-3 text-white px-1">Detailed Feedback</h3>
            <div className="space-y-3">
                {scorecard.map((item, index) => {
                    const scorePercentage = Math.round((item.score / item.max_score) * 100);
                    ...
                    return (
                        ...
                        >
                            ...
                            >
                                ...
                                <svg
                                    className={`w-5 h-5 text-zinc-400 transition-transform ${isExpanded ? 'transform rotate-180' : ''}`}
                                    xmlns="http://www.w3.org/2000/svg"
                                    viewBox="0 0 20 20"
                                    fill="currentColor"
                                >
                                ...
                                <div className="p-4 border-t border-zinc-800">
                                    <div className="space-y-3">
                                        {item.feedback.correct && (
                                            <div className="rounded-lg border border-emerald-900/30 bg-emerald-900/10 p-3">
                                                <div className="flex items-start">
                                                    ...
                                                    <div>
                                                        <h4 className="text-xs font-medium text-emerald-300 mb-1">Strengths</h4>
                                                        <p className="text-xs text-emerald-300/80">{item.feedback.correct}</p>
                                                    </div>
                                                </div>
                                            </div>
                                        )}
                                        ...
                                        {item.feedback.wrong && (
                                            <div className="rounded-lg border border-amber-900/30 bg-amber-900/10 p-3">
                                                <div className="flex items-start">
                                                    ...
                                                    <div>
                                                        <h4 className="text-xs font-medium text-amber-300 mb-1">Areas for improvement</h4>
                                                        <p className="text-xs text-amber-300/80">{item.feedback.wrong}</p>
                                                    </div>
                                                </div>
                                            </div>
                                        )}
